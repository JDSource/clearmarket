...
def main():
    ...
    user = (
        ...
        "Palette (inspiration, NOT a lookup table): odds climb, odds slip, holds near 50%, prices in, "
        "stays heavily favored, remains a long shot, nears full pricing, holds. PLAIN-ENGLISH TEST (hard requirement): the title must be understandable in ONE read by someone who has never seen a prediction market — the bar is a CNBC/Bloomberg news headline an anchor could read on air unchanged. Everyday verbs only: climbs, slips, holds, builds, returns, backs, prices in, puts long/short odds on, stays near 50%. BANNED financial-poetry verbs: solidifies, fractures, hardens, wavers, commands, absorbs, anchors, crystallizes, calcifies (and kin — if a verb wouldn't appear in a CNBC headline, don't use it). NEVER characterize WHO is trading — no 'institutional', 'retail', 'whale', 'smart money', 'desks': order flow is anonymous; report what the ODDS/PRICING did, never who did it. MAX 62 characters (hard limit — count them; the long stance tail is the usual cause, keep it 2-3 words). NO probability, NO volume, NO venue "
        "names, NO math symbols (≥, ≤, ~). EXCEPTION — round landmark percentages: the EXACT current price "
        "stays in telemetry, but 25% / 50% / 75% ARE allowed (encouraged) to state a threshold stance plainly: "
        "'Odds on X slip below 50%', 'holds under 25%' — never 'even odds' / 'quarter odds', a number is "
        "clearer. Claim-defining figures that DEFINE the market (a strike, level, "
        "or date in the question) ARE allowed — spell them out ('4 percent', '$150K', 'by June 30'); only the "
        "PROBABILITY and VOLUME are forbidden. NUMBER FORMAT: compact notation only — dollar PRICE LEVELS from the claim as $65K or $150K, non-dollar counts/index levels as 30K / 80K (the 24h trading VOLUME is telemetry — NEVER put a volume dollar figure in the title); NEVER spell out 'thousand', 'million', or 'billion' (write $1.5B, never 'one and a half billion'); the '$' sign itself is REQUIRED notation, not a banned math symbol — write '$100K', never the word 'dollar' ('dollar 100K' / '100 dollar range' are wrong). Snapshot only: frame the current pricing state, never a "
        "permanent/final real-world consequence. Do NOT invent a date/horizon absent from the question. "
        "VARIATION (you write the whole batch in one call): alternate Subject-first ('Bass stays favored...') and "
        ...
        "GOOD: 'Bass stays heavily favored in the LA mayoral race'; 'Markets firmly price a Fed pause "
