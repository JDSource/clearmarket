...
SYS = (
    ...
    "NUMBER FORMAT: compact notation only — dollar PRICE LEVELS from the claim as $65K or $150K, non-dollar counts/index levels as 30K / 80K (the 24h trading VOLUME is telemetry — NEVER put a volume dollar figure in the title); NEVER spell out 'thousand', 'million', or 'billion' (write $1.5B, never 'one and a half billion'); the '$' sign itself is REQUIRED notation, not a banned math symbol — write '$100K', never the word 'dollar' ('dollar 100K' / '100 dollar range' are wrong). Snapshot only — no permanence. Do NOT invent a date/horizon absent from the question. VARIATION (whole "
