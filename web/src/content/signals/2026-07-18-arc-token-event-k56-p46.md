---
signal_id: "CMSIG20260718DV01"
signal_slug: "arc-token-event-k56-p46"
headline: "Arc token launch before Jan 1, 2027: Kalshi 56% vs Polymarket 46%"
semantic_title: "Arc token launch timeline decouples on the major prediction desks"
telemetry: "Polymarket 46% vs Kalshi 56%"
category_tag: "CROSS_VENUE_DIVERGENCE"
detection_path: "cross_venue_divergence"
pre_news_classification: "concurrent"
published_at: "2026-07-18T09:21:12+00:00"
event_id: "CM-EVT-K7DR5FCFM0"
event_slug: "kxtokenlaunch-27jan01"
event_question: "Will Arc launch a token before Jan 1, 2027?"
primary_market:
  platform: "polymarket"
  platform_market_id: "0x0a8d89321f01639ec17b6bcf1ef0b6c15137b1934605f8bffeb50b2f291756d4"
  question_raw: "Will Arc launch a token by December 31 2026?"
  current_price: 0.46
  volume_cumulative_usd: 62271.83914700009
  arbitration_model: "uma_oracle"
  resolves_at: "2027-01-01T05:00:00Z"
related_markets:
  - platform: "kalshi"
    platform_market_id: "KXTOKENLAUNCH-27JAN01-ARC"
    question_raw: "Will Arc launch a token before Jan 1, 2027?"
    current_price: 0.56
bullets:
  - "Kalshi marks Arc token launch at 56%, Polymarket at 46%, a 10pp gap."
  - "Kalshi is higher; Polymarket holds roughly 24x the cumulative volume."
  - "Kalshi's thinner pool may be drawing from a tech-adjacent crowd more bullish on crypto project timelines."
  - "Resolves YES on any confirmed Arc token public launch before Jan 1, 2027."
atomic_claims:
  - type: "cross_venue_spread"
    provenance: "CM cross-venue link (question_id CMX-F36B8880EB); prices direct from venue APIs"
    field_provenance:
      kalshi_price:
        tier: "direct"
        method: "kalshi_api"
      poly_price:
        tier: "direct"
        method: "polymarket_clob_api"
      divergence_pp:
        tier: "derived"
        method: "arithmetic"
        inputs: ["kalshi_price", "poly_price"]
    liquidity_context:
      kalshi_vol_24h_usd: 3.94
      poly_vol_24h_usd: 122.859621
sources:
  - label: "ClearMarket cross-venue record: Will Arc launch a token before Jan 1, 2027?"
    url: "https://clearmarket.fyi/compare/arc-token-event-y-2026"
    retrieved_at: "2026-07-18T09:21:12+00:00"
field_provenance:
  pm_data: "kalshi_api, polymarket_clob_api"
  editorial_judgment: "cm_signal_llm_judge"
---

A 10pp dislocation on a shallow Kalshi market versus deep Polymarket flow points to a structural liquidity premium; the deeper venue's 46% is the more reliable anchor.
