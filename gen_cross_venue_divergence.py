...
SYS = (
    ...
    "(inspiration, NOT a lookup): splits sharply across venues, diverges, trades far apart on, converges, "
    "carries a premium on one venue. PLAIN-ENGLISH TEST (hard requirement): the title must be understandable in ONE read by someone who has never seen a prediction market — the bar is a CNBC/Bloomberg news headline an anchor could read on air unchanged. Everyday verbs only: climbs, slips, holds, builds, returns, backs, prices in, puts long/short odds on, stays near 50%. BANNED financial-poetry verbs: solidifies, fractures, hardens, wavers, commands, absorbs, anchors, crystallizes, calcifies (and kin — if a verb wouldn't appear in a CNBC headline, don't use it). NEVER characterize WHO is trading — no 'institutional', 'retail', 'whale', 'smart money', 'desks': order flow is anonymous; report what the ODDS/PRICING did, never who did it. MAX 62 characters (hard limit — count them; the long stance tail is the usual cause, keep it 2-3 words). Refer to venues GENERICALLY ('across venues', 'on the major desks') — do "
    "NOT name Kalshi/Polymarket or print either price (those live in telemetry). NO probability (round "
    "landmark percentages 25% / 50% / 75% allowed for a threshold stance — never 'even odds'), NO math "
    "symbols; claim-defining figures (the level/date in the claim) may be spelled out. NUMBER FORMAT: compact notation only — dollar PRICE LEVELS from the claim as $65K or $150K, non-dollar counts/index levels as 30K / 80K (the 24h trading VOLUME is telemetry — NEVER put a volume dollar figure in the title); NEVER spell out 'thousand', 'million', or 'billion' (write $1.5B, never 'one and a half billion'); the '$' sign itself is REQUIRED notation, not a banned math symbol — write '$100K', never the word 'dollar' ('dollar 100K' / '100 dollar range' are wrong). Snapshot only. Do NOT "
